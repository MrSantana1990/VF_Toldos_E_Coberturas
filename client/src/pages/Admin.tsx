...
export default function Admin() {
  ...
  const { data: stats, isLoading: statsLoading } = trpc.quotes.stats.useQuery(undefined, {
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-foreground/70">Total de Orçamentos</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-foreground/70">Orçamentos Pendentes</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-foreground/70">Orçamentos Concluídos</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-green-600">
                {statsLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : stats?.completed || 0}
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              {chartData.length > 0 ? (
                ...
                <div className="h-80 flex items-center justify-center text-foreground/50">Sem dados para exibir</div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between pb-4 border-b border-border">
                  <span className="text-sm text-foreground/70">Orçamentos Recebidos</span>
                  <span className="font-semibold">{stats?.total || 0}</span>
                </div>
                <div className="flex items-center justify-between pb-4 border-b border-border">
                  <span className="text-sm text-foreground/70">Aguardando Resposta</span>
                  <span className="font-semibold text-accent">{stats?.pending || 0}</span>
                </div>
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              ...
              >
                ...
                <p className="text-sm text-foreground/70">Gerenciar orçamentos recebidos</p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="text-sm text-foreground/70">Agendar visitas e instalações</p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="text-sm text-foreground/70">Controlar entradas e saídas</p>
              </button>
            </div>
          </CardContent>
        </Card>
